...
__title__ = "DIV MASS SETTER"
__doc__ = 'Заполнятор параметров по пересечению с выбранной формообразующей' \
...
class CategoryForm(Window):
	def __init__(self):
		wpf.LoadComponent(self, 'Z:\\pyRevit\\pyKPLN_KR (alpha)\\pyKPLN_KR.extension\\pyKPLN_KR.tab\\Проекты.panel\\Инструменты.pulldown\\btn_04.pushbutton\\PickCategories.xaml')
		self.Categories = []
...
class ParametersForm(Window):
	def __init__(self, parameters):
		wpf.LoadComponent(self, 'Z:\\pyRevit\\pyKPLN_KR (alpha)\\pyKPLN_KR.extension\\pyKPLN_KR.tab\\Проекты.panel\\Инструменты.pulldown\\btn_04.pushbutton\Parameters.xaml')
