...
__title__ = "СИТИ_Заполнение параметров 'ДИВ_Этаж_Текст', 'ДИВ_Секция_Текст'"
__author__ = 'Kapustin Roman'
__doc__ = "Автоматическое заполнение параметров для классификатора.\n"\
        "Заполянет параметры 'ДИВ_Этаж_Текст', 'ДИВ_Секция_Текст' с возможностью аналитического поиска секции и этажа для элементов, выходящих за оси здания.'\n"\
        "Актуально для всех разделов КР!"
...
# guid_grid_name = Guid("900729c1-070e-4e98-9a4a-c43559243fb6")

# get all elements of current project
categoryList = [BuiltInCategory.OST_StructuralFraming,BuiltInCategory.OST_StructuralColumns,
                BuiltInCategory.OST_GenericModel,BuiltInCategory.OST_Floors,BuiltInCategory.OST_Walls,
                BuiltInCategory.OST_StructuralFoundation,BuiltInCategory.OST_Roofs,BuiltInCategory.OST_MechanicalEquipment,BuiltInCategory.OST_Windows]
...
class ParametersForm(Window):
    ...
    flagSectStart = True
    def __init__(self):
        wpf.LoadComponent(self, 'Z:\\pyRevit\\pyKPLN_KR (alpha)\\pyKPLN_KR.extension\\pyKPLN_KR.tab\\Проекты.panel\\Инструменты.pulldown\\btn_18.pushbutton\\Form.xaml')
    def _onStartClick(self, sender, evant_args):
        self.flag = True
        self.Close()
    def checkBox_Unchecked1(self, sender, evant_args):
        self.flagLvlStart = False
    def checkBox_Checked1(self, sender, evant_args):
        self.flagLvlStart = True
    def checkBox_Unchecked2(self, sender, evant_args):
        self.flagSectStart = False
    def checkBox_Checked2(self, sender, evant_args):
        self.flagSectStart = True
...
if window.flag:
    ...
    # ___________________________________________________Генерация списка экземпляров класса LvlBox_______________________________________________________
        ...
        for SectForLvlBox in lvlCurSplited:
            lvlBoxObj = LvlBox()
            lvlBoxObj.MinPointZ = curLvl.Elevation - 0.33 #ЗАДАЕТСЯ СМЕЩЕНИЕ НА 200ММ ДЛЯ УЧЕТА ЭЛЕМЕНТОВ В СТЯЖКЕ
