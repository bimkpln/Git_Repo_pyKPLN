...
__doc__ = 'Копирование типоразмеров полов' \
...
class CreateWindow(Form):
	def __init__(self): 
		...
		self.lb2 = Label(Text = "Типоразмеры полов из выбранного проекта будут\nскопированы в активный проект с заменой по наименованию.")
	...
	def get_types(self, document):
		list = []
		for i in DB.FilteredElementCollector(document).OfCategory(DB.BuiltInCategory.OST_Floors).WhereElementIsElementType().ToElements():
			try:
