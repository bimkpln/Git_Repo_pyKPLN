...
"""
__author__ = 'KPLN'
__title__ = "Основа окна"
__doc__ = 'Запись имени типа стены, в которой находится окно\n«00_Основа_Элемента» - имя основы элемента' \
...
with db.Transaction(name='Запись параметров'):
	for element in DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Windows).WhereElementIsNotElementType():
		fam_name = element.Symbol.FamilyName
		if fam_name.startswith("120_") or fam_name.startswith("121_"):
			try:
