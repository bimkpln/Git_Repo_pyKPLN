...
"""
__author__ = 'KPLN'
__title__ = "Основа двери"
__doc__ = 'Запись имени типа стены, в которой находится дверь\n«00_Основа_Элемента» - имя основы элемента' \
...
with db.Transaction(name='Запись параметров'):
	for element in DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Doors).WhereElementIsNotElementType():
		fam_name = element.Symbol.FamilyName
		if fam_name.startswith("100_"):
			try:
