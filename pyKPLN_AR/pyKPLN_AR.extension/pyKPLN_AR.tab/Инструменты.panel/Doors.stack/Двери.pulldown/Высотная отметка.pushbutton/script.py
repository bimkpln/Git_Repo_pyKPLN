...
# Основные переменные
collector_elements = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Doors).WhereElementIsNotElementType()
...
with Transaction(doc, 'KPLN_Отметки. Запись отметок') as t:
    ...
    try:
        for element in collector_elements:
            ...
            if famName.startswith("100_"):
                b_box = element.get_BoundingBox(None)
                door_base = element.get_Parameter(BuiltInParameter.INSTANCE_SILL_HEIGHT_PARAM).AsDouble()
                ...
                if FindSharedParameter(element):
                    door_btm_hole = element.get_Parameter(guid_btm_hole).AsDouble()
                    temp = 0
                else:
                    try:
                        door_btm_hole = element.LookupParameter("Вырезание под проемом").AsDouble()
                        temp = door_btm_hole
                    except:
                        door_btm_hole = element.LookupParameter("Вырезание пола снизу").AsDouble()
                        temp = door_btm_hole

                value = "Низ на отм. " + GetDescription(door_base - door_btm_hole) + " мм от ур.ч.п."
