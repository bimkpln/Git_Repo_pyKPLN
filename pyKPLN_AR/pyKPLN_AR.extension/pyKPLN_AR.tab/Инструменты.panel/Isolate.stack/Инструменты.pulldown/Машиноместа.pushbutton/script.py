    ...
    if spline and len(elements) != 0:
        ...
        with db.Transaction(name = "mark"):
            for e in elements:
                ...
                pp = nurbSpline.Project(location).Parameter
                ##print(pp)
                
                """ xyzColl = nurbSpline.Project(location).XYZPoint
                list_elements.append(element)
                list_parameters.append(xyzColl)
                list_parameters_sorted.append(xyzColl) """
                #region
                opt = Options()
                ...
                for gObj in elemGeom:
                    ...
                    for solid in geomInsts:
                        ...
                        faces = solid.Faces
                        for face in faces:
                            setComparisonResult = face.Intersect(nurbSpline)
