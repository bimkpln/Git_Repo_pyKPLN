...
class CreateWindow(Form):
    def __init__(self): 
        ...
        if self.reNumber:
            ...
            self.cb1.Size = Size(343, 100)
            self.cb.Location = Point(30, 80)
    ...
    def allow_ok(self, sender, event):
        ...
        if self.value == "ПОМ_Номер помещения":
            self.reNumber = True
        ...
        for gObj in elemGeom:
            ...
            for face in faces:
                if type(face) == PlanarFace:
                    # if face.FaceNormal.X == 0 and face.FaceNormal.Y == 0 and face.FaceNormal.Z == -1: #Добавить перебор всех горизонтальных плоскостей (если хоть на одну проецируется точка)
