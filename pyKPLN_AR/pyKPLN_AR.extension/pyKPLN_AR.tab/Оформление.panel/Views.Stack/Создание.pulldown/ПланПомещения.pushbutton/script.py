...
__title__ = "План разверток"
__doc__ = 'Создание плана помещения для разверток и расположение созданного плана на отдельном листе.' \
...
class Picker(Form):
	def __init__(self):
		...
		for view_type in DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Views):
			if str(view_type.ViewType) == "FloorPlan" and view_type.IsTemplate:
	...
	def CblOnChanged(self, sender, event):
		...
		if self.pick_template.Text != "<выберите шаблон вида из списка>" and self.pick_sheet.Text != "<выберите формат листа из списка>":
			...
			for view_type in DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Views):
				if str(view_type.ViewType) == "FloorPlan" and str(view_type.Title) == self.pick_template.Text:
...
if len(rooms) != 0:
	if not script_scipped:
		with db.Transaction(name = "Create floor plans"):
			for room in rooms:
				room.CreateFloorPlanAndSheet(view_template, sheet_template)
