...
def set_name_number(sheet, prefix, separator, number, general):
    SheetNumber = ""
    try: 
        if not general:
        ...
        sheet.SheetNumber = '{} {} {}'.format(prefix, separator, sheet_number)
        try:
...
try:
    if len(sheets_sorted) == 0:
        ...
        components = [ui.forms.flexform.Label("[Том] - [Номер]\n"), ui.forms.flexform.Label("Том:"),ui.forms.flexform.TextBox('preffix', Text="АР1"), ui.forms.flexform.Label("Разделитель:"),ui.forms.flexform.TextBox('sep', Text="-"), ui.forms.flexform.Label("Начать номерацию с:"), ui.forms.flexform.TextBox('num', Text="1"), ui.forms.flexform.Label("Нумерация:"), ui.forms.flexform.CheckBox('general', '00# вместо #', default=False), ui.forms.flexform.Button("Запуск")]
    if len(sheets_sorted) != 0:
        form = ui.forms.FlexForm("Пронумеровать выбранный вид", components)
        form.ShowDialog()
        prefix = form.values['preffix']
        separator = form.values['sep']
        ...
        with db.Transaction('Переименовать вид TEMP'):
            for sheet in sheets_sorted:
                newsheets.append(set_name_number(sheet, "TEMP{}".format(str(sheet.Id.IntegerValue)), separator, number, False))
                number += 1
        number = int(suffix)
        with db.Transaction('Переименовать вид'):
            for sheet in newsheets:
                set_name_number(sheet, prefix, separator, number, general)
                number += 1
        dpId = DockablePanes.BuiltInDockablePanes.ProjectBrowser
        dP = DockablePane(dpId)
        dP.Show()
except: pass
