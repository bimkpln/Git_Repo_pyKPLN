...
__title__ = "Расчитать потолки"
__doc__ = 'Определение принадлежности элементов к помещениям.\n\n' \
          'Список параметров элементов:\n' \
          '«О_Имя помещения» - системный параметр\n ' \
          '«О_Назначение помещения» - системный параметр\n ' \
          '«О_Id помещения» - системный параметр\n ' \
		  '«О_Номер помещения» - системный параметр\n ' \
		  '«О_Номер секции» - номер секции из «ПОМ_Секция»\n ' \
...
def create_check_boxes_by_name(elements):
    ...
    elements_checkboxes = forms.SelectFromList.show(elements_options,
                                                    multiselect=True,
                                                    title='В твоей модели нет помещений. Выбери нужную связь:',
                                                    width=600,
                                                    button_name='Запуск!')
...
class CreateWindow(Form):
    def __init__(self): 
        ...
        #VARIABLES
        self.collector_rooms = rooms_collector_list
        self.collector_elements = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Ceilings).WhereElementIsNotElementType().ToElements()
        self.type_description = "CEILINGS"
        self.rooms = []
        self.elements = []
        ...
        self.gb.append(GroupBox())
        self.gb[1].Text = "Фильтр для потолков"
    ...
    def run(self, sender, args):
        ...
        if len(self.elements) != 0 and len(self.rooms) != 0:
            ...
            if len(result) == 0:
                with forms.ProgressBar(title='Initialization... | Detected {value} of {max_value}') as pb:
...
try:
    ...
    rooms_collector_list = list()
    if not rooms_collector:
        ...
        for link in linkModels:
            ...
            rooms_collector_list.extend(DB.FilteredElementCollector(linkDoc).
                                        OfCategory(DB.BuiltInCategory.OST_Rooms).
                                        WhereElementIsNotElementType().
                                        ToElements())
    else:
        rooms_collector_list = rooms_collector
    form = CreateWindow()
