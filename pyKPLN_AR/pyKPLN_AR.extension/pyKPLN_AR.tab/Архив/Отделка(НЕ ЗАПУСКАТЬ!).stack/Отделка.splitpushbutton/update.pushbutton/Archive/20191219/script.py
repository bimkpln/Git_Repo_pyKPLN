...
__title__ = "Обновить \n[модель]"
__doc__ = 'Запись расчетных значений отделки в помещения\n\n' \
...
class PreferencesWindow(Form):
	def __init__(self):
		...
		self.checkbox_calculate_results.Text = "Расчитывать итоговую площадь"
		...
		self.checkbox_add_numbers.Text = "Добавление номеров помещений к именам"
...
class ElementsStack():
	...
	def GetTypesIds(self):
		...
		return "_".join(types_ids_sorted)
		...
		for set in self.Elements:
			...
			for element in set:
				if not self.IsPlinth:
					...
					area += round(element.get_Parameter(DB.BuiltInParameter.HOST_AREA_COMPUTED).AsDouble() * 0.09290304 / GetType(element).LookupParameter("О_Плинтус_Высота").AsDouble() * 0.3048 * setting_k, 2)
				try:
			...
			#
			if not self.IsPlinth:
				self.InfoValue.append("{}Тип: «{}»\n{}м²{}".format(prefix, mark, str(area), space))
			else:
				self.InfoValue.append("{}Тип: «{}»\n{} пог.м{}".format(prefix, mark, str(area), space))
			self.CommonArea += area
		self.InfoValue.sort()
		self.InfoDescription.sort()
		if setting_calculate_result and len(self.Types_ids) > 1:
			if not self.IsPlinth:
				self.InfoValue.append("Итого: {}м²".format(str(self.CommonArea)))
			else:
				self.InfoValue.append("Итого: {} пог.м".format(str(self.CommonArea)))
...
class RoomFinish():
	...
	def GetKey(self, input=[]):
		...
		if len(input) == 0:
			...
			self.Key = add_key + "\n{}\n{}\n{}\n{}".format(self.Walls.GetTypesIds(), self.Floors.GetTypesIds(), self.Ceilings.GetTypesIds(), self.Plinths.GetTypesIds())
		return self.Key
	...
	def ApplyNull(self):
		try:
			self.Room.LookupParameter("О_ПОМ_Описание стен").Set("")
			self.Room.LookupParameter("О_ПОМ_Описание полов").Set("")
			self.Room.LookupParameter("О_ПОМ_Описание потолков").Set("")
			self.Room.LookupParameter("О_ПОМ_Описание плинтусов").Set("")
			self.Room.LookupParameter("О_ПОМ_Площадь стен_Текст").Set("")
			self.Room.LookupParameter("О_ПОМ_Площадь полов_Текст").Set("")
			self.Room.LookupParameter("О_ПОМ_Площадь потолков_Текст").Set("")
			self.Room.LookupParameter("О_ПОМ_Длина плинтусов_Текст").Set("")
			self.Room.LookupParameter("О_ПОМ_Площадь стен").Set(0.0)
			self.Room.LookupParameter("О_ПОМ_Площадь полов").Set(0.0)
			self.Room.LookupParameter("О_ПОМ_Площадь потолков").Set(0.0)
		except Exception as e: print((str(e)))

	def ApplyNullGrouped(self):
		for room in self.Rooms:
			try:
				room.LookupParameter("О_ПОМ_Группа").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Описание стен").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Описание полов").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Описание потолков").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Описание плинтусов").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь стен_Текст").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь полов_Текст").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь потолков_Текст").Set("")
				room.LookupParameter("О_ПОМ_ГОСТ_Длина плинтусов_Текст").Set("")
			except Exception as e: print((str(e)))

	def ApplySingle(self):		
		try:
			self.Room.LookupParameter("О_ПОМ_Описание стен").Set(self.Walls.GetDescription())
			self.Room.LookupParameter("О_ПОМ_Описание полов").Set(self.Floors.GetDescription())
			self.Room.LookupParameter("О_ПОМ_Описание потолков").Set(self.Ceilings.GetDescription())
			self.Room.LookupParameter("О_ПОМ_Описание плинтусов").Set(self.Plinths.GetDescription())
			self.Room.LookupParameter("О_ПОМ_Площадь стен_Текст").Set(self.Walls.GetValueDescription())
			self.Room.LookupParameter("О_ПОМ_Площадь полов_Текст").Set(self.Floors.GetValueDescription())
			self.Room.LookupParameter("О_ПОМ_Площадь потолков_Текст").Set(self.Ceilings.GetValueDescription())
			self.Room.LookupParameter("О_ПОМ_Длина плинтусов_Текст").Set(self.Plinths.GetValueDescription())
			self.Room.LookupParameter("О_ПОМ_Площадь стен").Set(self.Walls.GetCommonArea())
			self.Room.LookupParameter("О_ПОМ_Площадь полов").Set(self.Floors.GetCommonArea())
			self.Room.LookupParameter("О_ПОМ_Площадь потолков").Set(self.Ceilings.GetCommonArea())
		except Exception as e: print((str(e)))
	...
	def ApplyGrouped(self):
		...
		try:
			...
			if setting_add_numbers:
				group = "{}\n\n({})".format(", ".join(g_names_sorted), ", ".join(g_numbers_sorted))
			else:
				group = "{}".format(", ".join(g_names_sorted))
			for room in self.Rooms:
				room.LookupParameter("О_ПОМ_Группа").Set(group)
				room.LookupParameter("О_ПОМ_ГОСТ_Описание стен").Set(self.Walls.GetDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Описание полов").Set(self.Floors.GetDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Описание потолков").Set(self.Ceilings.GetDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Описание плинтусов").Set(self.Plinths.GetDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь стен_Текст").Set(self.Walls.GetValueDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь полов_Текст").Set(self.Floors.GetValueDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Площадь потолков_Текст").Set(self.Ceilings.GetValueDescription())
				room.LookupParameter("О_ПОМ_ГОСТ_Длина плинтусов_Текст").Set(self.Plinths.GetValueDescription())
		except Exception as e: print(str(e))
...
def GetStackGrouped(key):
	for i in range(0, len(rooms_grouped_key)):
		if key == rooms_grouped_key[i]:
			return rooms_grouped[i]
	return False
...
collectro_walls = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Walls).WhereElementIsNotElementType().ToElements()
...
collector_ceilings = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Ceilings).WhereElementIsNotElementType().ToElements()
			...
			for room_finish in rooms_all:
				...
				if not room_finish.IsEmpty():
					...
					if stack_exist == False:
						...
						ticker_max += 1
						rooms_grouped_key.append(room_finish.GetKey(setting_group_by))
					else:
