...
__title__ = "Расчитать полы (перекрытия)"
...
class CreateWindow(Form):
    def __init__(self):
        ...
        link_models = DB.FilteredElementCollector(doc).OfClass(DB.RevitLinkInstance)
        for link in link_models:
            self.collector_elements = DB.FilteredElementCollector(link.GetLinkDocument()).OfCategory(DB.BuiltInCategory.OST_Floors).WhereElementIsNotElementType().ToElements()
        self.type_description = "FLOORS"
        self.rooms = []
        self.elements = []
        ...
        self.gb.append(GroupBox())
        self.gb[1].Text = "Фильтр для полов"
	...
	def bb_intersection(self, element, room):
		try:
			...
			if room_max.Z < element_min.Z or room_min.Z > element_max.Z+10:
				return False
