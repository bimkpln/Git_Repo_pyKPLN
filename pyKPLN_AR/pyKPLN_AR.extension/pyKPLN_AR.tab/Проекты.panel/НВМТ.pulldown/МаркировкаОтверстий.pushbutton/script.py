...
class SSymbol():
    def __init__(self, name, width, height, offset, levElevation, elemElevation, defenition, element, host, islink):
        self.Type = name
    ...
    for element in collector_elements:
        ...
        if famName.startswith("199_Отверстие"):
            try:
                ...
                # Имя типа
                famDefenition = famSymb.get_Parameter(DB.BuiltInParameter.ALL_MODEL_TYPE_NAME).AsString()
                host = element.Host.Name.startswith("00_")
                symbols.append(SSymbol(name, width, height, offset, levElevation, elemElevation, famDefenition, element, host, isLink))
...
if ShowForm != "Отмена":
    ...
    if next:
        ...
        # По задаче от АР - нужно сортировать по "Ширина, высота, тип, отметка"
        ...
        with db.Transaction("KPLN. Нумерация отверстий"):
            ...
            for key, symbols in typesDict.items():
                # Сортировка по типу ВНУТРИ ключа
                symbols = sorted (symbols, key=lambda s: s.Defenition)
                cnt += 1
                for symbol in symbols:
