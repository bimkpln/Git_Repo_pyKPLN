...
class CreateWindow(Form):
	def __init__(self): 
		...
		self.abs_numeration = []
		self.rooms_collector = []
		if desOpts:
			# ПОМЕЩЕНИЯ ИЗ ВАРИАНТА
			self.rooms_collector.extend(DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Rooms).WherePasses(fRule_var).WhereElementIsNotElementType().ToElements())
			# ПОМЕЩЕНИЯ ИЗ ГЛАВНОЙ МОДЕЛИ
			self.rooms_collector.extend(DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Rooms).WherePasses(fRule_main).WhereElementIsNotElementType().ToElements())
		else:
	...
	def go_to_help(self, sender, args):
		webbrowser.open('https://kpln.kdb24.ru/article/60338/')
	...
	def is_studio(self, flat):
		for r in flat:
			if r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-гостиная" or r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-ниша":
				return True
			if r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-ниша" or r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-ниша":
				return True
			if r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "гостиная" or r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-ниша":
				return True
		return False
						...
						for n in range(0, len(self.rooms_names)):
							if name == self.rooms_names[n].split("+")[1]:
								if self.cb[n].Text == "Аренда":
									...
									self.tep_dou += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
								else: pass
				except: break
		#FLATS
										...
										if self.rooms_names[n].split("+")[0] == r.get_Parameter(self.builtin_room_department_par).AsString():
											if name == self.rooms_names[n].split("+")[1]:
												if self.cb[n].Text == "Кв: Лоджия":
													...
													rooms_balcony.append(r)
													if self.chbx_settings[0].Checked: 
														s_revit = round(r.Area * 0.09290304 * 0.3, 2)
														par = r.LookupParameter(par_area_room_k)
														par.Set(s_revit / 0.09290304)
														s_flat_k += s_revit / 0.09290304
														s_flat_balcony_k += s_revit / 0.09290304
...
if revit.doc.IsWorkshared:
    desOpts = DB.\
              ...
              OfCategory(DB.BuiltInCategory.OST_DesignOptions).\
              WhereElementIsNotElementType().\
              ToElements()
    if desOpts:
        desOptDict = {dO.get_Parameter(DB.BuiltInParameter.OPTION_NAME).AsString(): dO for dO in desOpts}
        comB = ui.forms.flexform.ComboBox
        lab = ui.forms.flexform.Label
        butt = ui.forms.flexform.Button
        separ = ui.forms.flexform.Separator
        components = [lab("Узел выбора варианта"),
                      lab("Выбери вариант:"),
                      comB("desOpt", desOptDict),
                      separ(),
                      butt("Далее")
                      ]
        form = ui.\
               forms.\
               FlexForm("Маркировка помещений или пространств", components)
        form.ShowDialog()
        desOpt = form.\
                 values["desOpt"].\
                 get_Parameter(DB.BuiltInParameter.OPTION_NAME).\
                 AsString()

        # УСЛОВИЕ ДЛЯ ПОМЕЩЕНИЙ ИЗ ВАРИАНТА
        bip = DB.BuiltInParameter.DESIGN_OPTION_PARAM
        provider = DB.ParameterValueProvider(DB.ElementId(bip))
        evaluater = DB.FilterStringContains()
        fStrRule = DB.FilterStringRule(provider,
                                    evaluater,
                                    desOpt,
                                    False)
        fRule_var = DB.ElementParameterFilter(fStrRule)

        # УСЛОВИЕ ДЛЯ ПОМЕЩЕНИЙ ИЗ ГЛАВНОЙ МОДЕЛИ
        fStrRule = DB.FilterStringRule(provider,
                                    evaluater,
                                    "Главная модель",
                                    False)
        fRule_main = DB.ElementParameterFilter(fStrRule)
