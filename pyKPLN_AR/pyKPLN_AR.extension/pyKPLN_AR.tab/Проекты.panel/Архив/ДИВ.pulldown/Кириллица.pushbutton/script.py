...
__title__ = "Поиск кириллицы-латинницы"
    ...
    try:
        for j in element.Parameters:
            if j.IsShared and j.Definition.Name == "ДИВ_RBS_Код по классификатору":
                value = j.AsString()
                if value != None:
                    if value.upper().startswith("HK") or value.upper().startswith("HК") or value.upper().startswith("НK"):
                        return [True, value]
    ...
    for category in doc.Settings.Categories:
        if not category.IsTagCategory:
            for element in DB.FilteredElementCollector(doc).OfCategoryId(category.Id).ToElements():
                ...
                if rslt[0]:
                    num+=1
                    print("\n[#{}]{}: {}\nЛатинские символы в параметре «ДИВ_RBS_Код по классификатору» - {}".format(str(num), out.linkify(element.Id), GetName(element), rslt[1]))
