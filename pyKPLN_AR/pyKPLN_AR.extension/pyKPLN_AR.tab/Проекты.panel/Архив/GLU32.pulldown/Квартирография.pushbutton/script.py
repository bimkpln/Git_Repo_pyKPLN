...
__title__ = "Квартирография (без евро)"
...
class CreateWindow(Form):
	...
	def is_studio(self, flat):
		for r in flat:
			if r.get_Parameter(self.builtin_room_name_par).AsString().lower() == "кухня-гостиная":
				return True
		return False

	def get_studio(self, flat):
		for r in flat:
						...
						with db.Transaction(name = "ta"):
							for i in range(0, len(self.dict_rooms_sorted)):
											...
											if name == self.rooms_names[n].split("+")[1]:
												if self.cb[n].Text == "Кв: Лоджия":
													...
													if self.chbx_settings[0].Checked: 
														...
														s_flat_fact += s_revit
								isStudio = False;
								if self.is_studio(self.dict_rooms_sorted[i]) and s_flat_living==0:
									isStudio = True;
									r = self.get_studio(self.dict_rooms_sorted[i])
									if self.chbx_settings[0].Checked:
										s_revit = round(r.Area * 0.09290304, 2)
										rooms_living.append(r)
										s_flat_living += s_revit / 0.09290304
								...
								if len(rooms_living) == 0:
									...
									if self.is_studio(self.dict_rooms_sorted[i]) and isStudio:
										for r in self.dict_rooms_sorted[i]:
											par = r.LookupParameter(par_flat_name)
											par.Set("C")
											par = r.LookupParameter(par_flat_description)
									...
									for r in self.dict_rooms_sorted[i]:
										par = r.LookupParameter(par_flat_name)
										par.Set("{}".format(str(len(rooms_living))))
										par = r.LookupParameter(par_flat_description)
