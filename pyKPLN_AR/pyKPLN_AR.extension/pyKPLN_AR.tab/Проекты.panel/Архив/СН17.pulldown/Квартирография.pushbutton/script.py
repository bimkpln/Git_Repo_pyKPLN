								...
								if len(rooms_living) == 0:
									...
									if self.is_studio(self.dict_rooms_sorted[i]):
										for r in self.dict_rooms_sorted[i]:
											...
											par = r.LookupParameter(par_flat_description)
											par.Set("Cтудия")
											par = r.LookupParameter('Вместимость')
											par.Set("Cтудия")
									else:
										for r in self.dict_rooms_sorted[i]:
											par = r.LookupParameter(par_flat_name)
											par.Set("1К")
											#Дополнительный параметр для СН17
											ex_par = r.LookupParameter("КВ_Количество жилых комнат")
											ex_par.Set("1")
											####
											par = r.LookupParameter(par_flat_description)
									...
									if not self.is_studio(self.dict_rooms_sorted[i]):
										for r in self.dict_rooms_sorted[i]:
											...
											#Дополнительный параметр для СН17
											ex_par = r.LookupParameter("КВ_Количество жилых комнат")
											ex_par.Set("{}".format(str(len(rooms_living))))
											####
											par = r.LookupParameter(par_flat_description)
											par.Set("{}-комнатная квартира".format(str(len(rooms_living))))
											par = r.LookupParameter('Вместимость')
											par.Set("{}-комнатная".format(str(len(rooms_living))))
										...
										for r in self.dict_rooms_sorted[i]:
											...
											par = r.LookupParameter('Вместимость')
											par.Set("{}-евро".format(str(len(rooms_living))))
