...
__doc__ = 'KPLN Квартирография разработана для автоматической нумерации помещений и определения показетелей площади. См. инструкцию по использованию скрипта внутри (значок «?»)\n' \
		  ...
		  '      ТЭП_Общая_площадь_к\n' \
		  '      ТЭП_МОП\n' \
		  '      ТЭП_Технические_помещения\n' \
		  '      ТЭП_Технические_пространства\n' \
		  '      ТЭП_Аренда\n' \
		  '      ТЭП_Кладовые\n' \
		  '      ТЭП_Количество_квартир\n' \
		  '      ТЭП_Количество_кладовых\n' \
...
class CreateWindow(Form):
	def __init__(self): 
		...
		self.paramlist = ["ТЭП_Жилая_Площадь", "ТЭП_Жилая_Площадь"]
		...
		for room in self.rooms_collector:
			...
			if not self.in_list(room.get_Parameter(self.builtin_room_name_par).AsString(), self.rooms_names):
				self.rooms_names.append(room.get_Parameter(self.builtin_room_name_par).AsString())
		...
		self.default_parameters = ["Помещение: Назначение",
							 ...
							 "Помещение: Внутреквартирный номер",
							 "Помещение: Номер",
							 "Помещение: S факт.",
							 "Помещение: S коэфф.",
							 "Квартира: Наименование",
							 "Квартира: Номер (абсолютный)",
							 "Квартира: Код",
							 "Квартира: S факт.",
							 "Квартира: S жилая",
							 "Квартира: S лоджий",
		...
		self.default_parameters_values = ["Помещение: Назначение",
							 ...
							 "Помещение: пом_этаж_номер",
							 "Помещение: ПОМ_Номер Квартиры",
							 "ПОМ_Номер Помещения",
							 "Номер",
							 "КП_П_Площадь",
							 "ПОМ_Квартира.комната.площадь_к",
							 "ПОМ_Квартира.имя",
							 "квартира_номер_абсолютный",
							 "ПОМ_Квартира.код",
							 "ПОМ_Квартира.площадь.общая",
							 "ПОМ_Квартира.площадь.жилая",
							 "ПОМ_Квартира.площадь.лоджий",
							 "ПОМ_Квартира.площадь.нежилых помещений",
							 "ПОМ_Квартира.площадь.общая_к"]

		self.default_project_parameters = ["ТЭП_Жилая_площадь",
									 "ТЭП_Общая_площадь",
									 "ТЭП_Общая_площадь_к",
									 "ТЭП_МОП",
									 "ТЭП_Технические_помещения",
		...
		self.default_settings = {"Округлять площадь до «0.01»" : True,
							 "Записать ТЭП" : False,
							 "Показывать log" : True,
							 "Расчитывать квартиры" : True,
							 "Расчитывать неквартирные помещения" : True,
							 "Автоматически создавать проверочные таблицы (при отсутствии) <в разработке>" : False}

		self.default_settings_names = ["Округлять площадь до «0.01»", "Записать ТЭП", "Показывать log",
								 "Расчитывать квартиры", "Расчитывать неквартирные помещения",
								 "Автоматически создавать проверочные таблицы (при отсутствии) <в разработке>"]
		...
		self.tooltip.SetToolTip(self.bt[0], "Открыть инструкцию в браузере «Notion.so»")
		...
		for i in range(0, len(self.rooms_names)):
			...
			self.cb[i].Location = Point(150, 50 * i + 5)
			...
			if self.rooms_names[i].lower() in "лоджиялоджии":
				self.cb[i].Text = self.default_categories[1]
			elif self.rooms_names[i].lower() in "ванная комнатапостирочная комнатадушдушеваясанузелтуалетс/ууборная":
				self.cb[i].Text = self.default_categories[5]
			elif self.rooms_names[i].lower() in "прихожаягардеробнаякухня-столоваякладовая":
				self.cb[i].Text = self.default_categories[4]
			elif self.rooms_names[i].lower() in "моплклестничная клеткалифтовой холллифтовый холлтамбурвходной тамбурколясочнаякоридорлк-0лк-1лк-2лк-3лк-4лк-5лк-6лк-7лк-8лк-9лк-10кл-11лк-12":
				self.cb[i].Text = self.default_categories[6]
			elif self.rooms_names[i].lower() in "террасатерасатеррасса":
				self.cb[i].Text = self.default_categories[2]
			elif self.rooms_names[i].lower() in "балконы":
				self.cb[i].Text = self.default_categories[3]
			elif self.rooms_names[i].lower() in "гостиннаягостинаякухня-гостинаяжилая комнатаспальняспальная комната":
				self.cb[i].Text = self.default_categories[0]
			elif self.rooms_names[i].lower() in "пуиитпкроссоваяврупомещение по сбору мусора":
				self.cb[i].Text = self.default_categories[7]
			else:
				self.cb[i].Text = self.default_categories[9]
		for i in range(0, len(self.rooms_names)):
			self.lbl_names.append(Label(Text = self.rooms_names[i]))
			self.lbl_names[i].Parent = self.pnl[0]
			self.lbl_names[i].Size = Size(145, 50)
			self.lbl_names[i].Location = Point(0, 50 * i + 5)
		...
		self.tooltip.SetToolTip(self.bt[2], "Открыть инструкцию в браузере «Notion.so»")
		...
		for i in range(0, len(self.default_settings.keys())):
			...
			if i == 5:
				self.chbx_settings[i].Enabled = False
	...
	def load_settings(self):
		try:
			...
			if os.path.exists(filepath):
				...
				try:
					self.commonarea_former = float(values[29])
				except: 
					self.commonarea_former = 0.00

				try:
					self.errors_amount_former = int(values[30])
				except: 
					self.errors_amount_former = 0
				...
				self.default_settings = {"Округлять площадь до «0.01»" : v[0],
									"Записать ТЭП" : v[1],
									"Показывать log" : v[2],
									"Расчитывать квартиры" : v[3],
									"Расчитывать неквартирные помещения" : v[4],
									"Автоматически создавать проверочные таблицы (при отсутствии) <в разработке>" : v[5]}
				for b in range(0, len(self.cb_parameters)):
					if  values[b + 10]:
						if values[b + 10] > "":
							self.cb_parameters[b].Text = values[b + 10]
				room_types_input = values[27].split("::")
				room_types = []
				...
				for t in room_types:
					for i in range(0, len(self.rooms_names)):
						if t[0] == self.rooms_names[i]:
							self.cb[i].Text = t[1]
				self.formula_parts_input = values[28].split("::")
				self.formula_parts_o_input = values[31].split("::")
				self.formula_parts = []
	...
	def save_settings_manual(self, override = True):
		try:
			...
			if not os.path.exists(path):
				os.makedirs(path)
				if self.chbx_settings[2].Checked: print("\t\tpath created: {}".format(path))
			else:
				if self.chbx_settings[2].Checked:
					print("\t\tpath already exist: {}".format(path))
			filepath = "{}\\rooms_settings.txt".format(path)
			if self.chbx_settings[2].Checked: print("\t\tattempt to open document: {}".format(filepath))
	...
	def logger(self, result):
		try:
			now = datetime.datetime.now()
			filename = "{}-{}-{}_{}-{}-{}_КВАРТИРОГРАФИЯ_{}.txt".format(str(now.year), str(now.month), str(now.day), str(now.hour), str(now.minute), str(now.second), revit.username)
	...
	def go_to_help(self, sender, args):
		webbrowser.open('https://www.notion.so/perfilevid/300180b60da94492a89e2ee53502c4d6')
	...
	def check_level(self, flats = list()):
		...
		for room in flats:
			if room == flats[0]:
				rooms += "{}".format(str(room.Id))
			else:
				rooms += ",{}".format(str(room.Id))
			if str(room.LevelId) != levelid:
				error = True
		if error:
			if self.chbx_settings[2].Checked: print("\tПредупреждение: FLAT№{} - помещения находятся на разных уровнях (ROOMS: {})".format(flatnum, rooms))
	...
	def generate_tep(self, otherrooms, flatrooms):
		...
		if self.chbx_settings[4].Checked:
			for room in otherrooms:
				...
				if not self.in_list(tep_department, tep_department_dict):
					...
					tep_department_dict_area.append(round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2))
				else:
					for i in range(0, len(tep_department_dict)):
						if tep_department == tep_department_dict[i]:
							tep_department_dict_area[i] += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
				self.tep_all += round(room.LookupParameter(self.cb_parameters[7].Text).AsDouble() * 0.09290304, 2)
				self.tep_all_k += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
				name = room.get_Parameter(self.builtin_room_name_par).AsString()
				if name and name != "":
					for n in range(0, len(self.rooms_names)):
						if name == self.rooms_names[n]:
							if self.cb[n].Text == "Аренда":
								self.tep_com += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
							elif self.cb[n].Text == "Тех.помещения":
								self.tep_tech += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
							elif self.cb[n].Text == "Тех.пространства":
								self.tep_tech_space += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
							elif self.cb[n].Text == "МОП":
								self.tep_mop += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
							elif self.cb[n].Text == "Кладовые":
								self.tep_store += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
		...
		if self.chbx_settings[3].Checked:
			for room in flatrooms:
				self.tep_all += round(room.LookupParameter(self.cb_parameters[7].Text).AsDouble() * 0.09290304, 2)
				self.tep_all_k += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
				self.tep_flats += round(room.LookupParameter(self.cb_parameters[7].Text).AsDouble() * 0.09290304, 2)
				self.tep_flats_k += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
				name = room.get_Parameter(self.builtin_room_name_par).AsString()
				for n in range(0, len(self.rooms_names)):
					if name == self.rooms_names[n]:
						if self.cb[n].Text == "Кв: Жилое пом.":
							self.tep_flats_living += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
						elif self.cb[n].Text == "Кв: Нежилое пом. (мокрое)":
							self.tep_flats_wet += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
						elif self.cb[n].Text == "Кв: Лоджия" or self.cb[n].Text == "Кв: Балкон":
							self.tep_flats_balcony += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
						elif self.cb[n].Text == "Кв: Терраса":
							self.tep_flats_terrass += round(room.LookupParameter(self.cb_parameters[8].Text).AsDouble() * 0.09290304, 2)
						else: pass
		#WRITE DATA
		...
		if self.chbx_settings[2].Checked: self.out.print_md('###TEP SAVING')
		if self.chbx_settings[2].Checked: print("\t\tВыберите путь для сохранения таблицы . . .")
		if (dialog.ShowDialog(self) == DialogResult.OK):
			filepath = "{}\\tep.txt".format(dialog.SelectedPath)
			if self.chbx_settings[2].Checked: print("\t\tpath: {}".format(filepath))
			file = open(filepath, "w+")
			file.write(self.tep.encode('utf-8'))
			file.close()
			if self.chbx_settings[2].Checked: print("\t\tsuccess : closing file")
		else:
			if self.chbx_settings[2].Checked: print("\t\tscipped")
	...
	def get_parameter_def(self, roomp, parameter):
		...
		elif parameter.startswith("Связанный уровень: "):
			level = doc.GetElement(roomp.LevelId)
			return str(level.LookupParameter(parameter[19:]).AsString())
		return "??"

	def run(self, sender, args):
		with forms.ProgressBar(title='initialising...') as pb:
			try:
				pb.update_progress(1, max_value = 8)
				pb.title = 'gathering information...'
				if self.check() and REVIT.query.get_central_path(doc=revit.doc) > "":
					self.Hide()
					if self.chbx_settings[2].Checked:
					...
					#GATHERINFORMATION
					if self.chbx_settings[2].Checked: self.out.print_md('###GATHERING INFORMATION')	
					if self.chbx_settings[2].Checked:
						print("\t\tdocument: {}".format(doc.PathName))
						print("\t\tcentral: {}".format(REVIT.query.get_central_path(doc=revit.doc)))
						print("\t\tuser: {}".format(revit.username))
						print("\t\tversion: {}".format(revit.version))
					for room in self.allrooms:
						...
						try:
							if self.get_parameter_def(room, self.cb_parameters[0].Text) != "":
								if room.Area > 0:
									if self.get_parameter_def(room, self.cb_parameters[0].Text) == "Кв":
										if self.chbx_settings[3].Checked:
								...
								if self.chbx_settings[2].Checked: print("\tПредупреждение: ROOM{}id- отсутствует назначение. Помещение будет исключено из расчета".format(str(room.Id)))
						except:
							if self.chbx_settings[2].Checked: print("\tROOM{}id- ошибка чтения параметров".format(str(room.Id)))
							error_param_rooms.append(room)
					if self.chbx_settings[2].Checked: self.out.print_md('###ROOMS')
					if self.chbx_settings[2].Checked: print("\t\tнеразмещенных / избыточных / неокруженных помещений - {}".format(str(len(unplaced_rooms))))
					if self.chbx_settings[2].Checked: print("\t\tпомещений без назначения - {}".format(str(len(unidentified_rooms))))
					if self.chbx_settings[2].Checked: print("\t\tошибок - {}".format(str(len(error_param_rooms))))
					if self.chbx_settings[2].Checked: print("\t\tквартирных помещений - {}".format(str(len(flat_rooms))))
					if self.chbx_settings[2].Checked: print("\t\tпрочих помещений - {}".format(str(len(other_rooms))))
					if self.chbx_settings[2].Checked: print("\n\t\tвсего помещений: {}".format(str(len(self.allrooms))))
					#NOT FLAT AREA
					pb.title = 'calculating other rooms...'
					pb.update_progress(3, max_value = 8)
					if self.chbx_settings[2].Checked: self.out.print_md('###NO FLAT ROOMS AREA AND NUMBER SET')
					if self.chbx_settings[4].Checked:
								...
								for n in range(0, len(self.rooms_names)):
									...
									if self.chbx_settings[0].Checked:
										if self.cb[n].Text == "Кв: Лоджия":
											s_revit = round(r.Area * 0.09290304 * 0.5, 2)
											...
											s_revit = round(r.Area * 0.09290304, 2)
											par = r.LookupParameter(par_area_room_k)
											par.Set(s_revit * 0.3 / 0.09290304)
											par = r.LookupParameter(par_area_room_fact)
											par.Set(s_revit / 0.09290304)
										else:
											s_revit = round(r.Area * 0.09290304, 2)
						...
						if self.chbx_settings[2].Checked: print("\tskipped")
					#GET DICTS
					pb.title = 'getting dicts...'
					pb.update_progress(4, max_value = 8)
					if self.chbx_settings[2].Checked: self.out.print_md('###GET DICTS')
					if self.chbx_settings[3].Checked:
						for r in flat_rooms:
						...
						if self.chbx_settings[2].Checked: print("\t\tколичество квартир: {}".format(str(len(self.abs_numeration))))
						...
						if self.chbx_settings[2].Checked: print("\tskipped")
					#GET FLAT AREA
					pb.title = 'calculating flatr rooms...'
					...
					if self.chbx_settings[3].Checked:
						with db.Transaction(name = "ta"):
							for i in range(0, len(self.dict_rooms_sorted)):
									...
									for n in range(0, len(self.rooms_names)):
										if name == self.rooms_names[n]:
											if self.cb[n].Text == "Кв: Лоджия":
												...
												k = 0.5
												if self.chbx_settings[0].Checked: 
													s_revit = round(r.Area * 0.09290304 * k, 2)
													par = r.LookupParameter(par_area_room_k)
													par.Set(s_revit / 0.09290304)
												...
												if self.chbx_settings[0].Checked: 
													...
													elif self.cb[n].Text.startswith("Кв: Нежилое пом."):
								...
								pb.title = 'error check...'
								pb.update_progress(6, max_value = 8)
								self.check_level(self.dict_rooms_sorted[i])
								if len(rooms_living) == 0:
									if self.chbx_settings[2].Checked: print("\t\tОшибка: FLAT№{} - отсутствуют жилые помещения!".format(self.dict_rooms_sorted[i][0].LookupParameter(par_flat_num_abs).AsString()))
									...
									if self.chbx_settings[2].Checked: print("\t\tПредупреждение: FLAT№{} - 5 жилых помещений".format(self.dict_rooms_sorted[i][0].LookupParameter(par_flat_num_abs).AsString()))
								elif len(rooms_living) > 5:
									for r in self.dict_rooms_sorted[i]:
										par = r.LookupParameter(par_flat_name)
										par.Set("NA")
									...
									if self.chbx_settings[2].Checked: print("\t\tПредупреждение: FLAT№{} - больше 5-ти жилых помещений".format(self.dict_rooms_sorted[i][0].LookupParameter(par_flat_num_abs).AsString()))
						...
						if self.chbx_settings[2].Checked: print("\tskipped flat area")
					#WRITE NUMBER
					pb.title = 'write num values...'
					pb.update_progress(7, max_value = 8)
					if self.chbx_settings[2].Checked: self.out.print_md('###WRITE NUMS')
					with db.Transaction(name = "write_final_number"):
						if self.chbx_settings[3].Checked:
								...
								for v in range(0, len(self.formula_parts)):
									if self.formula_parts[v] == "Нумерация: номер по назначению":
										...
										if v != 0:
											...
											if item == "": 
												item = "??"
												warning = "\t\tОшибка записи номера помещения: ROOM{}id отсутствует значение параметра «{}». Установлено по умолчанию: «??»".format(str(room.Id), self.formula_parts[v][6:])
										...
										if v != 0:
											...
											if item == "":
												item = "??"
												warning = "\t\tОшибка записи номера помещения: LEVEL{}id отсутствует значение параметра «{}». Установлено по умолчанию: «??» (ROOM{}id)".format(str(level.Id), self.formula_parts[v][5:], str(room.Id))
												if self.chbx_settings[2].Checked: print(warning)
											value += ".{}".format(item)
										else:
											item = level.LookupParameter(self.formula_parts[v][5:]).AsString()
							...
							if self.chbx_settings[2].Checked: print("\tskipped writing flat number")
						if self.chbx_settings[4].Checked:
								...
								for v in range(0, len(self.formula_parts_o)):
									if self.formula_parts_o[v] == "Нумерация: номер по назначению":
										...
										if v != 0:
											...
											if item == "": 
												item = "??"
												warning = "\t\tОшибка записи номера помещения: ROOM{}id отсутствует значение параметра «{}». Установлено по умолчанию: «??»".format(str(room.Id), self.formula_parts_o[v][6:])
												if self.chbx_settings[2].Checked: print(warning)
											value += ".{}".format(item)
										else:
											item = room.LookupParameter(self.formula_parts_o[v][6:]).AsString()
										...
										if v != 0:
											...
											if item == "":
												item = "??"
												warning = "\t\tОшибка записи номера помещения: LEVEL{}id отсутствует значение параметра «{}». Установлено по умолчанию: «??» (ROOM{}id)".format(str(level.Id), self.formula_parts_o[v][5:], str(room.Id))
												if self.chbx_settings[2].Checked: print(warning)
											value += ".{}".format(item)
										else:
											item = level.LookupParameter(self.formula_parts_o[v][5:]).AsString()
							...
							if self.chbx_settings[2].Checked: print("\tskipped writing no flat number")
					...
					if self.errors_amount == 0:
						...
						result = "\t\tОшибок - {}".format(str(self.errors_amount))
					if self.chbx_settings[2].Checked: print(result)
					now = datetime.datetime.now()
					...
					if self.chbx_settings[2].Checked: self.out.print_md('###AUTOSAVING CURRENT SETTINGS')
					self.save_settings_manual()
					if self.chbx_settings[2].Checked: self.out.print_md('###UPDATING DATA')
					self.load_settings()
					if self.chbx_settings[2].Checked: self.out.print_md('###DONE!')
					self.logger(result)
					pb.update_progress(8, max_value = 8)
					pb.title = 'done!'
					if self.chbx_settings[2].Checked: self.out.print_md('###LOG SAVED')
					self.show_alert("", "{}!".format(result))
					self.Show()
			except: 
				self.save_settings_manual(override = False)
				self.load_settings()
				self.logger("failed")
				self.show_alert("Окно будет закрыто", "Неизвестная ошибка")
				self.Close()
				self.Show()
				return
