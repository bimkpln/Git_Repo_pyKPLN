...
def Add():
    ...
    exCount = 0
    outList = list()
    ui.forms.Alert('Выберите элемент, который нужно вставить, а затем элемент, стоящий после него!', title="Внимение!")
    while True:
    ...
    while True:
        selectedReference = ui.Pick.pick_element(msg='Выбери следующий элемент. По окончанию - нажми "Esc"!')
        NextPickedEl = doc.GetElement(selectedReference.id)
        if 'Марки'.lower() not in NextPickedEl.Category.Name.lower() and str(NextPickedEl.Category.Name.lower()) != 'оси' and str(NextPickedEl.Category.Name.lower()) != 'связанные файлы'  and str(NextPickedEl.Category.Name.lower()) != 'линии':
            break
    ind = 0
    for element in newListSys:
        ...
        for el in curEls:
            curEl = el
        if curEl.Id == NextPickedEl.Id:
            TrueInd = ind
            exSys = element
            panel = exSys.BaseEquipment
            Pref = element.get_Parameter(systemParam).AsString()
            exSys.SelectPanel(PickedEl)
            
        ind+=1
    sys =  ElectricalSystem.Create(doc, [PickedEl.Id], ElectricalSystemType.FireAlarm)
    sys.SelectPanel(panel)
    sys.get_Parameter(ElAdressParam).Set(1)
    sys.get_Parameter(systemParam).Set(Pref)
    newListSys.insert(TrueInd, sys)
